...
- **A site holding the basket without ShopBundle no longer 500s** on `/shop/basket/display`: the basket pages called `c975LShop:Shop:ViewButton` and `c975LShop:Product:Recommendations`, components of a bundle this one only suggests (31/08/2026)
- ShopBundle is the surcouche of this bundle, not the reverse - nothing here names a shop route, a shop component or a shop template any more (31/08/2026)
- **New `CatalogueBasketItemProviderInterface`**, optional beside `BasketItemProviderInterface`: `getCatalogueUrl(): ?string`, the listing the "continue shopping" button sends the customer back to, generated by the selling bundle itself (31/08/2026)
- `BasketItemProviderRegistry::getCatalogueUrl()` takes the first listing installed; with none, the button is not drawn rather than pointing at a route no site declares (31/08/2026)
- New `payment_catalogue_url()` Twig function, what `Basket:ContinueShoppingButton` is drawn from (31/08/2026)
- `Basket:ContinueShoppingButton` replaces the five calls to ShopBundle's own `Shop:ViewButton`, one button for the basket rather than two doing the same (31/08/2026)
- **`BasketRecommendationProviderInterface::getTemplate()`**, the template drawing what `getRecommendations()` returns - the entries are the provider's own entities, so is the markup showing them (31/08/2026) [BC-Break] see [UPGRADE.md](UPGRADE.md)
- `BasketRecommendationRegistry::getTemplate()` answers that of the same first provider, null when none is installed, the strip then being left out (31/08/2026)
- The download links of the confirmation e-mail carry their own address, built by the bundle owning the route, `Basket:DownloadLinks` laying out what it is handed as `Basket:Downloads` already did (31/08/2026) [BC-Break] see [UPGRADE.md](UPGRADE.md)
- The README, `UPGRADE.md` and the `c975l-payment-items` and `c975l-payment-checkout` skills state the catalogue, the recommendation template and the links the download e-mail carries (31/08/2026)
