...
selling out of one also implements `c975L\PaymentBundle\Contract\CatalogueBasketItemProviderInterface`, whose
single `getCatalogueUrl(): ?string` returns its address — generated by the provider itself, which is the only one
to know the parameters and the fragment its own listing takes. Null when the catalogue is not reachable for now.
...
A provider offering cross-sell implements `c975L\PaymentBundle\Contract\BasketRecommendationProviderInterface`:
`getRecommendations(Basket $basket, int $limit): array` returns **its own entities**, and `getTemplate(): string`
names the template drawing them, handed those entries as a `recommendations` variable and nothing else. The markup
belongs to whichever bundle recommends — this one only says where the strip goes on the page. Only one provider is
expected, the first registered winning; with none, the strip is left out.
