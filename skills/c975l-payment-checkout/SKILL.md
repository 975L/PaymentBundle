...
**Delivery is read off a grid an admin writes in the back office, and there is no flat rate any more** — `shop-shipping` is gone. A `ShippingZone` groups the countries posted at one tariff and carries its weight tiers (`ShippingRate`), both edited on one screen (`ShippingZoneCrudController`). `ShippingRateResolverInterface::resolve()` charges a parcel at the **first tier it fits in**, that tier's ceiling included; the tier with no ceiling catches everything above the others. The zone naming **no country** is the catch-all (`ShippingZone::isCatchAll()`), where a country named in no other zone falls.

**Nothing written is nothing charged.** No zone, no catch-all, or no tier covering the parcel, and `resolve()` answers `null` — which the basket charges as zero. That is deliberate and it is silent, so `ShippingHealthCheckProvider` (kind `payment-shipping`, site-wide) reports an empty grid, a missing or duplicated catch-all zone, and a zone whose tiers all stop short.

`Service\PaymentDemoFixtureProvider` writes a grid to look at rather than one to imagine, for whatever demo dataset the application installs: three zones — the country the shop is in, everything around it, and the catch-all taking the rest — each with its weight tiers. **The third one is not decoration**: a grid shipped without it reports a warning on the very screen a demo shows, and ships free to every country the first two miss.

The parcel's weight is summed in `updateTotals()` from the lines whose provider implements `WeighableBasketItemProviderInterface` (see `c975l-payment-items`); a line answering `null` adds nothing. **The packaging is never weighed** — a tier is written for the parcel the carrier puts on the scales.

**The basket page shows an estimate, and the checkout charges the real thing.** The country only exists once the customer gives an address, so the basket is priced on `shop-shipping-country` until then; `validate()` runs `updateTotals()` again once `CoordinatesType` has bound the address, and **refuses the order rather than charging a total no page ever showed** — the fresh shipping and the fresh discount are each compared with what the customer was last shown, the basket is flushed with its true amounts, and a `BasketNotOrderableException` sends them back to the basket that now says them. The promotional code goes through the same gate: `assertCodeStillHolds()` reads the totals of *before* the refresh, so a code falling short only on the fresh ones is caught there.

`CoordinatesType` asks for the country in a list and stores the **ISO 3166-1 alpha-2 code** — a zone naming `FR` recognises nothing in "france". Orders already placed keep whatever free text they carry: they are frozen snapshots, never priced again.

A page stating what delivery starts at reads `payment_shipping_from()` (`Twig\ShippingExtension`), the lowest price the whole grid holds — `null` on a grid with no tier, where the block says nothing rather than promising free delivery.

**The `payment_shipping` block is cached, and the grid is what drops it.** UiBundle's own listener only ever invalidates the `Block` that was edited, and knows nothing of what that block reads at render time. `Service\PaymentBlockCacheTagProvider` therefore puts an extra tag on the kind's entry through `BlockCacheTagProviderInterface`, and `Listener\PaymentCacheInvalidationListener` calls `PaymentBlockCacheInvalidator::invalidateShipping()` whenever a `ShippingZone` or a `ShippingRate` is written, or a `Config` row named in its own `CONFIG_SLUGS` — `shop-shipping-free` and `shop-currency`, the only two settings the block reads. The settings are dropped on `postFlush` and not per row: the back office saves the whole group at once.
...
The free-shipping threshold is weighed against **what the basket holds**, not what is paid: a ten-euro code must not cost the customer the shipping they had earned. `shop-shipping-free` left unset is **no threshold at all**, and no longer "free from zero": read as an amount it made the comparison false on every basket, so a shop that had never set one was charging no delivery whatever its grid says.
...
**Business details, not e-invoicing.** `Basket::$company` and `$vatNumber` (stored upper-cased, spaces and dots
dropped) are asked by `CoordinatesType`, which requires the full address once a company is given
(`validateCompany()`) and, on `SUBMIT`, drops the VAT number — and a digital order's address — when the company is
emptied (`forgetBusiness()`). `BasketService::create()` prefills them from `BasketRepository::findLastBusinessByUser()`.
A Factur-X e-invoice — PDF/A-3 with its XML inside, through an approved platform — is still not what this is.
...
The basket, its validation and the two account pages answer a bare url and a localised twin — `basket_display_localized`, `basket_validate_localized`, `customer_orders_localized` and `customer_order_localized`, at `/{_locale}/...` — in **every** language the site declares: a basket and an order hold the visitor's own choices, and what is read around them is this bundle's catalogues. Each action runs ConfigBundle's `LocalizedRouteNegotiator` (`redirectToAskedLanguage()`, then `vary()`), and the three ways back to the basket out of `validate()` name the twin when the request carries a `_locale`.

**The token urls and the invoice have no twin.** A payment link, a shared order or an unsubscribe link is read in the language the order was placed in (`Basket::$locale`), and the invoice in the issuing company's.

Templates link these screens with `localized_path()`. A link stored in content goes through `Service\PaymentLinkLocalizer` (UiBundle's `InternalLinkLocalizerInterface`), which rewrites `/shop/basket/display`, `/shop/basket/validate` and `/account/orders` alone — a query string or an anchor travelling along, a further segment being another screen and left as it is.
...
**The `download_information` email carries its links ready-made.** Whichever bundle sends it passes `downloadLinks` as `['title' => ..., 'url' => ..., 'size' => ...]` per file — the address built by the bundle owning the download route, as `BasketDownloadProviderInterface` already did for the order page. `Basket:DownloadLinks` lays out what it is handed and resolves nothing.

**`shop-email-bcc` is the shop's only record of what went out**, and left empty it is silent — the email simply leaves without its blind copy. `Management\PaymentAlertProvider` therefore warns on the dashboard while it is empty, linking to the **payment** group of the config listing - the six `shop-email-*` keys sit there, being the shop's own senders and not the site's, which is what the `email` group holds.
...
**Every page this bundle serves a visitor answers `noindex`**, set as `{% set robots = ... %}` in the template and read by the layout: the basket and the unsubscribe page keep `follow` so their links back to the products still pass on, the token-reached and account pages take `nofollow`. Each is one visitor's own, and an indexed copy is either an empty cart standing for a page of the shop or an order handed to whoever searched for it.
...
- **Do not add a page to this bundle without its `robots`** — every page it serves a visitor is one visitor's own, and none belongs in an index.
- **Do not weigh the free-shipping threshold against what is paid** rather than what the basket holds, and do not read an unset `shop-shipping-free` as a threshold of zero.
...
- **Do not place `Basket:Navbar` in a site layout, a page or a block** — UiBundle's layout already does, and a second bar is never filled.
...
- **Do not link the basket, its validation or the order pages with `path()`** in a template — `localized_path()` keeps the visitor in the language they are reading.
- **Do not give a token url or the invoice a localised twin** — they are read in the order's language, not the reader's.
