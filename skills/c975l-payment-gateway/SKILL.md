...
`verifyCredentials()` returns null when the keys authenticate, the provider's own reason when they do not. **It asks for nothing the bundle does not already need**: `StripeGateway` lists a single Checkout Session rather than reading the account, so a restricted key scoped to Checkout alone passes — reading the account would be refused for lack of a permission no payment ever uses, and a site charging perfectly well would be reported as broken. It is called from `c975l:health-check:run` **only** — never from a controller, it reaches a third party and no dashboard page may block on that. `PaymentAlertProvider` reads the config and can only tell a *missing* key from a present one; a revoked or mistyped key reads exactly like a working one, which is what the health check exists for.
