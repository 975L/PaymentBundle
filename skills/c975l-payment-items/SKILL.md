...
A bundle showing a price of its own — a product sheet, a pack of credits — wraps the figure in the **`price`** class this bundle's stylesheet ships, adding **`price--standalone`** when the price stands alone under what it prices rather than in a row. Both are tuned through `--price-font-family`, `--price-font-weight`, `--price-size` and `--price-standalone-margin`, so a site restyles every bundle's prices at once. The word said above the figure — "Price", "From" — goes in a **`price-label`**, quieter than the figure and tuned through `--price-label-color` and `--price-label-size`.
