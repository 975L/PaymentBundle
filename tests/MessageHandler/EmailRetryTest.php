...
class EmailRetryTest extends TestCase
{
    ...
    {
        $handler = new ConfirmOrderMessageHandler($this->repository(), $this->sender(false));
    ...
    {
        $handler = new ItemsShippedMessageHandler($this->repository(), $this->sender(false));
    ...
    {
        $handler = new ConfirmOrderMessageHandler($this->repository(), $this->sender(true));
    ...
    public function testAVanishedBasketIsNotRetried(): void
    {
        ...
        $handler = new ConfirmOrderMessageHandler($repository, $this->sender(false));
    ...
    private function sender(bool $sent): BasketEmailSender
    {
        $sender = $this->createStub(BasketEmailSender::class);
        $sender->method('send')->willReturn($sent);
        $sender->method('getLastError')->willReturn('mailer refused the message');
