...
class SkillsTest extends TestCase
{
    ...
    private const string SLUG_PATTERN = '#^(payment|stripe|shop|url)-[a-z-]+$#';

    private const string NAME_PATTERN = '#^[a-z][a-z0-9]*(_[a-z0-9]+)+(\([^)]*\))?$#';

    private const string COMMAND_PATTERN = '/c975l:[a-z0-9:-]+/';

    private const string COMPONENT_PATTERN = '/<twig:c975LPayment:([A-Za-z:]+)/';
    ...
    public function testEveryCommandQuotedExists(): void
    {
        ...
        foreach ($this->loadSkills() as $directory => $content) {
            ...
            foreach (array_unique($matches[0]) as $command) {
                ...
                // This bundle names its own command under the c975l:shop: prefix, so the pattern above cannot be scoped to a prefix of its own - a command it does not declare is checked against the ecosystem, exactly as a config slug is
                $this->assertTrue($this->isDeclaredSomewhere($command), sprintf('%s quotes "%s", which no command of the ecosystem answers to', $directory, $command));
            }
        }
    }
